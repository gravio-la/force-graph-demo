...
import { Database, ChevronDown, Clock, FileText } from "lucide-react";
...
export function GraphSelector() {
  ...
  const setAvailableGraphs = useGraphStore((state) => state.setAvailableGraphs);
  const loadGraph = useGraphStore((state) => state.loadGraph);

  // Initialize available graphs on mount 
  useEffect(() => {
            ...
            >
              <div className="flex flex-col gap-1 flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1 text-xs text-muted-foreground pl-6 mt-1">
                  ...
                  <span>{graph.metadata?.timestamp ? formatTimestamp(graph.metadata.timestamp) : ''}</span>
                  {(graph.graph?.nodes || graph.graph?.links) && (
                    <>
                      <span className="mx-1">•</span>
                      <span>
                        {graph.graph?.nodes?.length || 0} nodes, {graph.graph?.links?.length || 0} links
                      </span>
                    </>
                  )}
                </div>
